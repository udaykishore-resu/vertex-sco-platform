// vertex-devicegateway (formerly deviceServerCTM) — terminal-tier Vertex service, running as a
// DaemonSet on each physical SCO lane.
//
// Hardware abstraction layer: talks JPOS/XML.POS to physical devices (scanner, scale, printer, cash, trilight) and exposes them to store-tier services over the event bus.
//
